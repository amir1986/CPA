...
export default async function TraceDetailPage({ params }: Props) {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <h1 className="mb-1 text-xl font-semibold">{tr("traces.trace")}</h1>
      ...
      <section className="mb-4 rounded-lg border border-border bg-bg p-4">
        <h2 className="mb-1 text-sm font-medium">{tr("common.question")}</h2>
        <p className="text-sm">{run.request}</p>
      </section>

      <h2 className="mb-2 text-sm font-medium">{tr("common.tool_calls")}</h2>
        ...
        {run.tool_calls.map((tc, i) => (
          <li key={i} className="rounded-lg border border-border bg-bg p-3">
            <div className="flex items-center justify-between">
              ...
              {tc.error && <span className="rounded-pill bg-danger/10 px-2 py-0.5 text-xs text-danger">{tr("traces.error")}</span>}
            </div>
...
{JSON.stringify({ arguments: tc.arguments, result: tc.result, error: tc.error }, null, 2)}
      ...
      <h2 className="mt-6 mb-2 text-sm font-medium">{tr("common.final_answer")}</h2>
      <p className="rounded-lg border border-border bg-bg p-4 text-sm whitespace-pre-wrap">
        {run.final_answer ?? tr("common.no_final_answer")}
      </p>
    </div>
  );
}
