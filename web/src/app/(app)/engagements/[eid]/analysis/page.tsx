...
export default async function AnalysisPage({ params, searchParams }: Props) {
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <h1 className="mb-1 text-xl font-semibold">{tr("analysis.title")}</h1>
      <p className="mb-4 text-sm text-fg-muted">{tr("analysis.subtitle")}</p>

      <section className="mb-6">
        <h2 className="mb-2 text-sm font-medium">{tr("analysis.ratios_period", { period })}</h2>
      ...
      <section>
        <h2 className="mb-2 text-sm font-medium">{tr("analysis.benford_title")}</h2>
        {!benford && <p className="text-sm text-fg-muted">{tr("analysis.benford_empty")}</p>}
        {benford && (
          <div className="rounded-lg border border-border bg-bg p-4">
            <div className="mb-3 flex items-center justify-between text-sm">
              ...
              <span className={
                ...
                {benford.suspect ? tr("analysis.anomalous") : tr("analysis.consistent")}
              </span>
            </div>
                ...
                return (
                  <div key={d} className="flex flex-col items-center">
                    <div className="flex h-24 items-end gap-0.5">
                      <div title={`${tr("analysis.observed")} ${obs.toFixed(1)}%`} className="w-3 bg-brand" style={{ height: `${obs * 3}px` }} />
                      <div title={`${tr("analysis.expected")} ${exp.toFixed(1)}%`} className="w-3 bg-fg-subtle/50" style={{ height: `${exp * 3}px` }} />
                    </div>
            ...
            <p className="mt-2 text-xs text-fg-muted">{tr("analysis.legend")}</p>
          </div>
        )}
      </section>
    </div>
